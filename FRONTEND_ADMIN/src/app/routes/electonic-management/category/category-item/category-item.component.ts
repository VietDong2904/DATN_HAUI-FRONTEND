...
export class CategoryItemComponent implements OnInit {
  ...
  updateFormToEdit(): void {
    ...
    this.form.get('isDisplay')?.enable();
  ...
  public initData(data: any, type: any = null, option: any = {}): void {
    ...
    if (this.item.id === null || this.item.id === undefined) {
      this.form = this.fb.group({
        ...
        isDisplay: [{ value: false, disabled: this.isInfo }],
      ...
      this.form = this.fb.group({
        ...
        isDisplay: [{ value: this.item.isDisplay, disabled: this.isInfo }],
        parentId: [{ value: this.item.parentId, disabled: this.isInfo }],
  ...
  resetForm(): void {
    this.form.reset();
    this.form.get('isDisplay')?.setValue(false);
    this.fecthListCategory();
  }
  ...
  save(isCreateAfter: boolean = false): Subscription | undefined {
    ...
    let data = {
      ...
      isDisplay: this.form.controls.isDisplay.value,
    ...
    if (this.isAdd) {
      ...
      const promise = this.categoryService.create(data).subscribe(
        (res: any) => {
          this.isLoading = false;
          if (res.code !== 200) {
      ...
      const promise = this.categoryService.update(data).subscribe(
        (res: any) => {
          this.isLoading = false;
          if (res.code !== 200) {
            this.messageService.error(`${res.message}`);
