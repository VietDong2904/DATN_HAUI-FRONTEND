...
import { CategoryService } from 'src/app/services/electronic-management/category/category.service';
...
export class CategoryComponent implements OnInit {
  ...
  @ViewChild(CategoryItemComponent, { static: false }) itemModal!: { initData: (arg0: {}, arg1: string) => void };
  ...
  deleteListItem(listId: [string]): Subscription {
    ...
    const promise = this.categoryService.delete(listId).subscribe(
      (res: any) => {
  ...
  initGridData(): Subscription {
    ...
    const rs = this.categoryService.getFilter(this.filter).subscribe(
      (res: any) => {
