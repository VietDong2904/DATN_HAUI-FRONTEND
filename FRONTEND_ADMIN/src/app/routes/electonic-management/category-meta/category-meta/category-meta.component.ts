...
import { CategoryMetaItemComponent } from '../category-meta-item/category-meta-item.component';
import { CategoryService } from '@service';

@Component({
  selector: 'app-category-meta',
  templateUrl: './category-meta.component.html',
  styleUrls: ['./category-meta.component.less'],
})
export class CategoryMetaComponent implements OnInit {
  ...
  @ViewChild(CategoryMetaItemComponent, { static: false }) itemModal!: { initData: (arg0: {}, arg1: string) => void };
  ...
  deleteListItem(listId: [string]): Subscription {
    ...
    const promise = this.categoryMetaService.delete(listId).subscribe(
      (res: any) => {
  ...
  initGridData(): Subscription {
    ...
    const rs = this.categoryMetaService.getFilter(this.filter).subscribe(
      (res: any) => {
