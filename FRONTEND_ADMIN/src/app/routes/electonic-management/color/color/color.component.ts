...
import { TagItemComponent } from '../color-item/color-item.component';
...
export class TagComponent implements OnInit {
  ...
  @ViewChild(TagItemComponent, { static: false }) itemModal!: { initData: (arg0: {}, arg1: string) => void };
  ...
  deleteListItem(listId: [string]): Subscription {
    ...
    const promise = this.colorService.delete(listId).subscribe(
      (res: any) => {
  ...
  initGridData(): Subscription {
    ...
    const rs = this.colorService.getFilter(this.filter).subscribe(
      (res: any) => {
