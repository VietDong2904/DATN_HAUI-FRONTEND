...
import { UnitImportItemComponent } from '../unit-import-item/unit-import-item.component';
...
export class UnitComponent implements OnInit {
  ...
  @ViewChild(UnitItemComponent, { static: false }) itemModal!: { initData: (arg0: {}, arg1: string) => void };
  ...
  @ViewChild(UnitImportItemComponent, { static: false }) importModal!: { initData: () => void };
  ...
  deleteListItem(listId: [string]): Subscription {
    ...
    const promise = this.unitApiService.delete(listId).subscribe(
      (res: any) => {
  ...
  initGridData(): Subscription {
    ...
    const rs = this.unitApiService.getFilter(this.filter).subscribe(
      (res: any) => {
